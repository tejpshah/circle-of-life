...
def agent6(num_simulations, nodes=50):
    ...
    for _ in range(num_simulations):
        game = Game(nodes)
        game_success, game_found_pred = game.run_agent_6()
...
def agent8(num_simulations, nodes=50):
    ...
    for _ in range(num_simulations):
        game = Game(nodes)
        game_success, game_found_prey, game_found_pred = game.run_agent_8()
...
def agent8C(num_simulations, nodes=50):
    """
    run simulation n times and get statistics on success
    """
    agent_success = []
    ...
    for _ in range(num_simulations):
        game = Game(nodes)
        game_success, game_found_prey, game_found_pred = game.run_agent_8C()
    ...
    success = wins/(len(agent_success))
    print(
        f"Agent8C: Wins: {wins}\tLosses: {losses}\tTimeouts: {timeouts}\tSuccess Rate: {round(success*100,2)}%")
    return wins, losses, timeouts, round(success*100, 2), found_prey/num_simulations, found_pred/num_simulations
...
def agent10(num_simulations, nodes=50):
    ...
    for _ in range(num_simulations):
        game = Game(nodes)
        game_success, game_found_prey, game_found_pred = game.run_agent_10()
    ...
    success = wins/(len(agent_success))
    print(
        f"Agent10: Wins: {wins}\tLosses: {losses}\tTimeouts: {timeouts}\tSuccess Rate: {round(success*100,2)}%")
    return wins, losses, timeouts, round(success*100, 2), found_prey/num_simulations, found_pred/num_simulations
...
def visualize_three_agents(dirname, filename):
    ...
    # create the bar graph with error bars
    x_pos = np.arange(len(agents))
    colors = ["lightcoral", "yellowgreen", "cadetblue"]
