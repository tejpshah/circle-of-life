...
class Agent6(Agent2):
    ...
    def move(self, graph, prey, predator):
        ...
        for key, val in distances.items():
            if val == min_dist: action = key 
        
        potential_predator_distance = self.bfs(graph, self.location, potential_predator.location)
        ...
        if potential_predator_distance > 4 or self.beliefs[action] <= 0.1: 
            ...
            distances = {} 
            for nbr in graph.nbrs[self.location]:
                distances[nbr] = self.bfs(graph, nbr, potential_predator.location)
            max_dist = max(distances.values())

            action = 0
            for key, val in distances.items():
                if val == max_dist: action = key 
            self.location = action  
